+// ▼ Unit-Test 用 API（この 3 行だけ！） -------------------------
+#ifdef UNIT_TEST
+void ResetAll();                    // 内部状態をすべてリセット
+void SimulateMove(const int targetRow);  // 行番号を指定して 1 ステップ進める
+void SimulateHalfStep();            // 半グリッド動かすテスト用
+#endi//+------------------------------------------------------------------+
...
#define UNIT_TEST                    // ① ここで一度だけ宣言
#ifdef UNIT_TEST                  // ② Fake API はこのブロック内でのみ有効
...
// 既に他の export と同じ領域（グローバル関数セクション）に置く
export void SimulateMove(const int targetRow)
{
   ...
   while(lastRow != targetRow && safety-- > 0)
   {
      ...
      PrintFormat("DBG SimMove: about to StepRow  r=%d  lastRow(before)=%d  dir=%d", next, lastRow, dir);
      StepRow(next, dir);
